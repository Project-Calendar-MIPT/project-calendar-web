...
const ProjectDetailPage: React.FC = () => {
  ...
  const loadProject = async () => {
    try {
      ...
      const allAssignments = assignmentService
        .getMockAssignments()
        .filter((assignment) => relatedTasks.some((task) => task.id === assignment.task_id));

      const uniqueUsers = new Set(allAssignments.map((assignment) => assignment.user_id));
